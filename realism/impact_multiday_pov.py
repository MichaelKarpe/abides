...
def concat_horizontal(df, s, resample_rate=RESAMPLE_RATE):
    ...
    df_out = pd.merge(
        df.resample(resample_rate).last(),
        s.resample(resample_rate).last(),
        how="outer",
        left_index=True,
        right_index=True,
    )
