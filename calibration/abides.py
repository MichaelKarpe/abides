...
def abides(name, agents, oracle, latency_model, default_computation_delay):
    ...
    agents_saved_states = kernel.runner(
        agents=agents,
        startTime=pd.Timestamp(f"{calibration.config.DATE} {calibration.config.START_TIME}")
        - pd.to_timedelta("00:01:00"),
        stopTime=pd.Timestamp(f"{calibration.config.DATE} {calibration.config.STOP_TIME}")
        + pd.to_timedelta("00:01:00"),
