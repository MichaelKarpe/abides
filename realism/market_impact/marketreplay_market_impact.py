...
def process_data(abides_log_folder, stock, date):
    ...
    ob_df.index = pd.read_pickle(abides_log_folder + f'ORDERBOOK_{stock}_FREQ_ALL_{date.replace("-", "")}.bz2').index[
        1:
    ]
    ...
    for index, row in ea_df.iterrows():
        transacted_orders_df = transacted_orders_df.append(
            pd.Series(
                data={
                    "TIMESTAMP": index,
...
def calculate_market_impact(orders_df, ob_df, start_time, end_time, tao):
    ...
    binned_buy_volume = create_bins(
        tao=int(tao), start_time=start_time, end_time=end_time, orders_df=orders_df, is_buy=True
    ).fillna(0)
    binned_sell_volume = create_bins(
        tao=int(tao), start_time=start_time, end_time=end_time, orders_df=orders_df, is_buy=False
    ).fillna(0)
...
if __name__ == "__main__":
    ...
    ob_df, orders_df, start_time, end_time = process_data(abides_log_folder=abides_log_folder, stock=stock, date=date)
