...
def calculate_market_impact(transacted_orders_df, orderbook_df, start_time, end_time, tao):
    ...
    def create_bins(tao, start_time, end_time, transacted_orders_df, is_buy):
        ...
        binned_volume = (
            transacted_orders_df.loc[transacted_orders_df.BUY_SELL_FLAG == is_buy].groupby(binned).SIZE.agg(np.sum)
        )
    ...
    binned_buy_volume = create_bins(
        tao=int(tao), start_time=start_time, end_time=end_time, transacted_orders_df=transacted_orders_df, is_buy=True
    ).fillna(0)
    binned_sell_volume = create_bins(
        tao=int(tao), start_time=start_time, end_time=end_time, transacted_orders_df=transacted_orders_df, is_buy=False
    ).fillna(0)
...
if __name__ == "__main__":
    ...
    orderbook_df = get_orderbook(
        abides_log_folder=abides_log_folder,
