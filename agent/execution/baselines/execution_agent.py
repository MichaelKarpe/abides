...
class ExecutionAgent(TradingAgent):
    ...
    def handleOrderExecution(self, currentTime, msg):
        ...
        log_print(
            "[---- {} - {} ----]: LIMIT ORDER EXECUTED - {} @ {}".format(
                self.name, currentTime, executed_order.quantity, executed_order.fill_price
            )
        )
        ...
        log_print(
            "[---- {} - {} ----]: % EXECUTED: {} \n".format(
                self.name, currentTime, round((1 - self.rem_quantity / self.quantity) * 100, 2)
            )
        )
    ...
    def placeOrders(self, currentTime):
        if currentTime == self.execution_time_horizon[-2]:
            ...
            if currentTime == self.start_time:
                ...
                log_print(
                    "[---- {}  - {} ----]: Arrival Mid Price {}".format(self.name, currentTime, self.arrival_price)
                )
