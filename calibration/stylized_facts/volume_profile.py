...
def generate_volume_profile(executed_orders_dict, freq="1min"):
    ...
    vol_profile = pd.DataFrame(
        columns=MARKET_REPLAY_DATES[::-1], index=pd.date_range(start="09:30:00", end="16:00:00", freq=freq).time
    )
...
def plot_transacted_volume(security, executed_orders_dict, freq="1min"):
    ...
    fig.suptitle(
        f"Transacted Volumes for {security} between {MARKET_REPLAY_DATES[-1]} and {MARKET_REPLAY_DATES[0]}",
        size=24,
        fontweight="bold",
    )
    for i, date in enumerate(executed_orders_dict):
    ...
    fig.savefig(
        f"transacted_volume_{security}_{freq}_bins.png",
        format="png",
        dpi=300,
        transparent=False,
        bbox_inches="tight",
...
def plot_cumulative_volume(security, executed_orders_dict, freq="1min"):
    ...
    fig.set_size_inches(30, 20)
    fig.suptitle(
        f"Cumulative Transacted Volumes for {security} between {MARKET_REPLAY_DATES[-1]} and {MARKET_REPLAY_DATES[0]}",
        size=24,
        fontweight="bold",
    )
    for i, date in enumerate(executed_orders_dict):
    ...
    fig.subplots_adjust(top=0.92)
    fig.savefig(
        f"cumulative_transacted_volume_{security}_{freq}_bins.png",
        format="png",
        dpi=300,
        transparent=False,
        bbox_inches="tight",
...
def plot_volume_profile(security, vol_profile):
    ...
    fig.suptitle(
        f"Volume Profile for {security}, raw data (left) and smoothed data (right) using a Gaussian Kernel "
        f"Smoother",
        size=24,
        fontweight="bold",
    )
    ...
    fig.savefig(
        f"volume_profile_{security}_{freq}_bins.png",
        format="png",
        dpi=300,
        transparent=False,
        bbox_inches="tight",
...
if __name__ == "__main__":
    ...
    for date in executed_orders_dict:
        executed_orders_dict[date] = get_transacted_orders(
            data_folder + f"marketreplay_{security}_{date}/EXCHANGE_AGENT.bz2"
        )
