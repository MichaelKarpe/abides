...
DISCRETE_SPACE_GRID = {"low": [0.1, 0.1, 1, 0.1], "high": [1.0, 1.0, 20, 1.0], "bins": (5, 5, 5, 5)}
...
class TabularQLearningAgent(TradingAgent):
    ...
    def __init__(
        ...
        self.wakeup_times = pd.date_range(
        ...
        self.discrete_state_grid = create_uniform_grid(
            low=DISCRETE_SPACE_GRID["low"], high=DISCRETE_SPACE_GRID["high"], bins=DISCRETE_SPACE_GRID["bins"]
        )
    ...
    def receiveMessage(self, current_time, msg):
        ...
        if msg.body["msg"] == "ORDER_EXECUTED":
            ...
            self.start_time <= current_time <= self.wakeup_times[-1].floor(self.freq)
    ...
    def step(self, action):
        ...
        if self.currentTime.floor(self.freq) == self.start_time:
            ...
            self.agent_log(
                msg="Parent Order Details: {} {} between {} and {}, "
                "every {} minutes".format(
                    self.direction, self.quantity, self.start_time, self.end_time, self.freq.seconds / 60
                ),
                type="info",
            )
        ...
        self.agent_log(
            msg="s = {}, a = {}, next_state = {}, r = {}".format(self.s, action, next_state, reward), type="info"
        )
    ...
    def act(self, next_state):
        ...
        def catchup_market_order():
            if self.remaining_parent_quantity > 0 and self.currentTime.floor(self.freq) == self.duration[-2]:
                self.placeMarketOrder(
                    symbol=self.security, direction=self.direction, quantity=self.remaining_parent_quantity
                )
        ...
        if a == 0:
            ...
            try:
                ...
                self.placeLimitOrder(
                    symbol=self.security, quantity=child_qty, is_buy_order=self.direction == "BUY", limit_price=price
                )
            except IndexError:
                self.placeLimitOrder(
                ...
                self.agent_log(
                    "ORDERBOOK SIDE EMPTY!!!!!!. placing limit order of price equal to last trade price!", type="error"
                )
                self.agent_log("bids: {}, asks: {}".format(bids[:10], asks[:10]), type="error")
            self.agent_log(
                msg="Limit order submitted - a={} "
                "price level: {}, q={}, p={}".format(a, a - 1, child_qty, self.last_trade_price)
            )
        return a
    ...
    def observe(self, bids, asks):
        ...
        def volume_order_imbalance(best_bid_size, best_ask_size):
            imb = (
                best_ask_size / (best_bid_size + best_ask_size)
    ...
    def log_results(self):
        ...
        total_pnl = self.arrival_price * total_executed_volume - sum(
            executed_order.fill_price * executed_order.quantity for executed_order in self.executed_orders
        )
        msg = (
            "Final Results: PnL: {} Reward {}, "
            "Arrival Price {}, Avg Txn Price {}, Difference: {}, "
            "Remaining Quantity {}".format(
                total_pnl,
                self.episode_reward,
                self.arrival_price,
        ...
        difference = (
            ...
            if self.direction == "BUY"
            else self.get_average_transaction_price() - self.arrival_price
        )
