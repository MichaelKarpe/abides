...
def get_orderbook(
    abides_log_folder, csv_orderbooks_parent_folder, security, date, start_time, end_time, num_price_levels
):
    """
    ...
    orderbook_df.index = pd.read_pickle(
        abides_log_folder + f'ORDERBOOK_{security}_FREQ_ALL_{date.replace("-", "")}.bz2'
    ).index[1:]
