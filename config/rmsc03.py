...
parser.add_argument("--start-time", default="09:30:00", type=parse, help="Starting time of simulation.")
parser.add_argument("--end-time", default="11:30:00", type=parse, help="Ending time of simulation.")
parser.add_argument(
...
parser.add_argument("-e", "--execution-agents", action="store_true", help="Flag to allow the execution agent to trade.")
parser.add_argument(
    "-p", "--execution-pov", type=float, default=0.1, help="Participation of Volume level for execution agent"
)
# market maker config
parser.add_argument("--mm-pov", type=float, default=0.025)
parser.add_argument("--mm-window-size", type=util.validate_window_size, default="adaptive")
parser.add_argument("--mm-min-order-size", type=int, default=1)
parser.add_argument("--mm-num-ticks", type=int, default=10)
parser.add_argument("--mm-wake-up-freq", type=str, default="10S")
parser.add_argument("--mm-skew-beta", type=float, default=0)
...
parser.add_argument("--fund-vol", type=float, default=1e-8, help="Volatility of fundamental time series.")
...
kernel = Kernel(
    "RMSC03 Kernel", random_state=np.random.RandomState(seed=np.random.randint(low=0, high=2 ** 32, dtype="uint64"))
)
...
latency_model = LatencyModel(latency_model="deterministic", random_state=latency_rstate, kwargs=model_args)
# KERNEL
